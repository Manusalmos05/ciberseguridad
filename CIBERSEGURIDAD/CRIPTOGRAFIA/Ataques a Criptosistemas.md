---

type: concept  
title: "Ataques a Criptosistemas"  
date: "2026-09-25"  
category:  
tags:

- cybersecurity
    

---

# 🧠 Ataques a Criptosistemas

## ¿Qué es?

Están expuestos a ataques en los cuales un los problemas de seguridad suelen ser por los elementos del mismo sistema que ya conoce un atacante.

---

## 🎯 ¿Para qué sirve?

Podemos preveer los puntos debiles y reforzar la seguridad

---

## 🔑 Conceptos importantes
#### Modelos de ataques teóricos:
-Ciphertext-Only attacks: conoce algoritmo de cifrado y algunos textos cifrados.
-Known-plaintext attacks: conoce algoritmo de cifrado y algunos pares de texto plano y cifrado
-Chosen-plaintext attacks: conoce algoritmo de cifrado y algunos pares de texto plano y cifrado, de los cuales ha podido seleccionar texto plano.
-Chosen-ciphertext attacks: conoce algoritmo de cifrado y algunos pares de texto plano y cifrado, de los cuales ha podido seleccionar texto plano y/o cifrado.

---

## 💡 Ejemplo

...

---

## 🔐 Aplicación en ciberseguridad

...

---

## ⚠️ Riesgos / vulnerabilidades

...

---

## 🧪 ¿Puedo practicarlo?

-  Laboratorio creado
    
-  Laboratorio completado
    
-  Documentado en GitHub
    

---

## 🔗 Conceptos relacionados

- [[]]
    
- [[]]
    
- [[]]
    

---

## ❓ Preguntas que todavía tengo

---

## 📚 Fuentes